...
const loadTabEvents = (windowManager: EngineWindowManager) => {
  ...
  const createTab = (win: EngineWindow) => (url: string) => {
    ...
    const loadStartHandler = () => {
      win.browserWindow.webContents.send(RendererEvents.TAB_LOAD_START, tab.id);
    };

    const loadStopHandler = () => {
      win.browserWindow.webContents.send(RendererEvents.TAB_LOAD_STOP, tab.id);
    };
